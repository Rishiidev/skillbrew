...
// The marketplace primitive: install skills from any skillbrew-format repo.
// spec: github:user/repo | full git URL | local path
export function install(spec, { pack = null, force = false } = {}) {
  if (!spec) throw new Error('usage: skillbrew install <github:user/repo | url | path> [--pack <name>] [--force]');
  let src = spec;
  ...
  let tmp = null;
  if (!exists(src)) {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'skillbrew-install-'));
  ...
  try {
    ...
    if (!exists(skillsDir)) throw new Error(`No skills/ directory in ${spec} — not a skillbrew-format repo`);
    ...
    console.log(`skillbrew: installed ${installed} skills from ${spec} — restart Claude Code to load them`);
  } finally {
    if (tmp) rmrf(tmp);
  }
}
