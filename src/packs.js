...
export function packCreate(name, skills) {
  if (!name || !skills.length) throw new Error('usage: skillbrew pack create <name> <skill...>');
  const repo = repoDir();
  ...
  if (missing.length) {
    throw new Error(`Not in last snapshot: ${missing.join(', ')}. Run skillbrew snapshot first or check names with skillbrew list.`);
  }
  manifest.packs = manifest.packs || {};
  manifest.packs[name] = skills;
  saveManifest(repo, manifest);
  console.log(`skillbrew: pack "${name}" = ${skills.join(', ')}`);
}

export function packList() {
  const manifest = loadManifest(repoDir());
  const packs = Object.entries(manifest.packs || {});
  if (!packs.length) return console.log('skillbrew: no packs yet — skillbrew pack create <name> <skill...>');
...
export function packInstall(name) {
  if (!name) throw new Error('usage: skillbrew pack install <name>');
  restore({ pack: name });
}
